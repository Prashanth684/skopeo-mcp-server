...
import (
	"errors"
	"fmt"
	"github.com/Prashanth684/skopeo-mcp-server/pkg/mcp"
...
var rootCmd = &cobra.Command{
	Use:   "skopeo-mcp-server [command] [options]",
	Short: "skopeo Model Context Protocol (MCP) server",
	Long: `
skopeo Model Context Protocol (MCP) server

  # show this help
  skopeo-mcp-server -h

  # shows version information
  skopeo-mcp-server --version

  # start STDIO server
  skopeo-mcp-server

  # start a SSE server on port 8080
  skopeo-mcp-server --sse-port 8080

  # start a SSE server on port 8443 with a public HTTPS host of example.com
  skopeo-mcp-server --sse-port 8443 --sse-base-url https://example.com:8443
